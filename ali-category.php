...
if(!alibaba_check_category_id($_GET['id'])){ _redirect301('/', true); }

$CURRENT_PAGE= current_page();
$addpages= (!empty($CURRENT_PAGE)) ? $CURRENT_PAGE : 1;
$CATS_ID= $_GET['id'];
$url= 'https://m.alibaba.com/catalogs/--'.$CATS_ID.'/'.$addpages.'?XPJAX=1&_='.time().'&ctoken=139euk04t2a6k';
...
//SET METADATA
$META_TITLE= $array['config']['cateName'];
...
<div class="sheader"><a href="<?php echo HOME_BASE_URL;?>">Home</a> <?php echo url_category($META_refineCateIds, '|');?> <?php echo $META_TITLE;?></div>	
         <div class="itemLists">		 
<h2 class="mainLnk title"><a href="/catid/<?php echo $CATS_ID;?>">Best Product On <?php echo $META_TITLE;?> Category</a></h2>
...
$CURRENT_PAGErun= $CURRENT_PAGE+5;	
	for($i=$CURRENT_PAGE+1; $i <=$CURRENT_PAGErun; $i++ ){
	 echo '<li><a href="/catid/'.$CATS_ID.'?page='.$i.'">'.$i.'</a></li>';
	}
}			
?>
                </ul>
...
echo alibaba_cookies(3, 'index-category', ALIBABA_OFFER);
}
?>
</body>
</html>
